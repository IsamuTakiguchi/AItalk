      ...
      <section className="rounded-2xl bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 flex gap-2">
          <button
            ...
            disabled={index === 0}
            className="flex-1 rounded-xl border border-ink-400/25 py-3 text-sm font-semibold text-ink-600 active:scale-[0.98] disabled:opacity-40"
          >
            戻る
          </button>
          {isLast ? (
            <Link
              to={`/courses/${courseId}`}
              onClick={finish}
              className="flex-1 rounded-xl bg-brand-600 py-3 text-center text-sm font-semibold text-white active:scale-[0.98]"
            >
              練習をはじめる
            </Link>
          ) : (
  ...
  return (
    <div className="mt-3 rounded-xl bg-brand-50 p-3">
      ...
      <button
        ...
        className="mt-2 rounded-lg bg-white px-3 py-2 text-xs font-semibold text-brand-700 active:scale-95"
      >
        🔈 読み上げる
      </button>
    </div>
